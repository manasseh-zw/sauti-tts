...
"""
Modal launcher for Stage 1 multi-speaker Shona F5-TTS full fine-tuning.
...
It converts the multi-speaker Shona export into the F5-TTS training format,
writes a run-local config, and then launches the existing scripts/train.py
entrypoint. Speaker identity is preserved in prep summaries for analysis, but
the Stage 1 training dataset is intentionally language-focused and uses the
standard F5 `audio_file|text` metadata format.
"""
...
APP_NAME = "sna-f5-tts-shona-base"
PYTHON_VERSION = "3.10"
GPU_TYPE = "A10G"
...
REMOTE_MODEL_ROOT = Path("/model/sna-f5-tts/shona-base")

DEFAULT_DATASET_SUBDIR = "wav_normalised"
DEFAULT_METADATA_FILENAME = "metadata_normalized.csv"
PREP_SUBDIR = Path("f5_prepared/default")

DEFAULT_RUN_NAME = "shona_base_v1"
DEFAULT_WANDB_PROJECT = "sna-f5-tts"
DEFAULT_BASE_CONFIG = "configs/finetune_single_gpu.yaml"
DEFAULT_EPOCHS = 3
DEFAULT_SAMPLE_EVERY_UPDATES = 1000
DEFAULT_SAMPLE_EVERY_EPOCHS = 1
...
def _dataset_root(dataset_subdir: str) -> Path:
    return REMOTE_DATA_ROOT / dataset_subdir.strip("/ ")


def _prepared_dataset_dir(dataset_subdir: str) -> Path:
    return _dataset_root(dataset_subdir) / PREP_SUBDIR


def _run_dir(run_name: str) -> Path:
    return REMOTE_MODEL_ROOT / run_name
    ...
    print(
        "\n".join(
            [
                ...
                f"  total_duration_hours={summary.get('total_duration_hours', '?')}",
                f"  num_speakers={summary.get('num_speakers', '?')}",
...
def _resolve_audio_path(dataset_root: Path, raw_path: str) -> Path:
    ...
    basename = dataset_root / candidate.name
    if basename.exists():
        return basename
...
def _validate_dataset_export(dataset_root: Path, metadata_filename: str) -> Path:
...
def _prepare_dataset(
    ...
    dataset_root = _dataset_root(dataset_subdir)
    metadata_path = _validate_dataset_export(dataset_root, metadata_filename)
    prep_dir = _prepared_dataset_dir(dataset_subdir)
    ...
    with open(metadata_path, "r", encoding="utf-8") as f:
        ...
        if missing_columns:
            raise ValueError(
                f"Dataset metadata is missing required columns: {missing_columns}"
            )
        ...
        for row in reader:
            ...
            transcript = _clean_transcript(row.get("transcription", ""))
            if not transcript:
                empty_transcript += 1
                continue
            ...
            audio_path = _resolve_audio_path(dataset_root, file_name)
            if not audio_path.exists():
                missing_audio += 1
                continue

            speaker_id = str(row.get("speaker_id", "")).strip() or "unknown"
    ...
    summary = {
        ...
        "total_duration_hours": round(total_duration_hours, 3),
        "num_speakers": len(speaker_counter),
        "speaker_distribution_top10": speaker_counter.most_common(10),
...
def _write_generated_config(
    *,
    ...
    dataset_label = dataset_subdir.strip("/ ").replace("/", "_") or "dataset"
    effective_wandb_run_name = wandb_run_name or f"{dataset_label}-{run_name}"
    logger_type = "wandb" if os.getenv("WANDB_API_KEY") else "tensorboard"

    config["exp_name"] = f"sna_f5_tts_{dataset_label}_{run_name}"
    config["output_dir"] = str(checkpoints_dir)

    dataset_cfg = config.setdefault("dataset", {})
    dataset_cfg["name"] = dataset_label
    ...
    training_cfg["batch_size_per_gpu"] = _recommended_batch_frames(
        prep_dir,
        int(training_cfg.get("batch_size_per_gpu", 1600)),
    )
    max_samples = int(training_cfg.get("max_samples", 64))
...
def run_f5_shona_base(
    ...
    summary: dict[str, Any] = {
        "dataset_subdir": dataset_subdir,
        "dataset_root": str(_dataset_root(dataset_subdir)),
